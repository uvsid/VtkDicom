//US: 2016-05-22: Built upon the FourPanelViewer VTK Example. 
...
QtVTKRenderWindows::QtVTKRenderWindows(int vtkNotUsed(argc), char *argv[])
{
	...
	connect(ui->btnOpenDicom, SIGNAL(clicked()), this, SLOT(LoadDicom()));
	this->ui->view1->setEnabled(false);
	this->ui->view2->setEnabled(false);
	this->ui->view3->setEnabled(false);
	this->ui->view4->setEnabled(false);
...
bool QtVTKRenderWindows::LoadDicom()
{
	vtkSmartPointer<vtkDICOMImageReader> dicomReader = vtkSmartPointer<vtkDICOMImageReader>::New();
	QString sDicomPath = "";
	// Get the Dicom folder path
	sDicomPath = QFileDialog::getExistingDirectory(this, (QString) "Dicom Folder Please", QDir::currentPath(), QFileDialog::ShowDirsOnly);
	if (sDicomPath == "") return false;

	
	dicomReader->SetDirectoryName(sDicomPath.toStdString().c_str());

	dicomReader->Update();

	this->ui->view4->setEnabled(true);
